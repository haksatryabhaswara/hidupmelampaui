...
import { INITIAL_SCORING, type ScriScoring } from "@/lib/scri-data";
...
const SCRI_PENILAIAN_STEPS: CopilotStep[] = [
  { id: "info", title: "Info Penilaian", desc: "Referensi: skor maksimal 180 (36 pertanyaan × skala 1–5). Gunakan ini sebagai acuan saat mengatur batas rentang skor." },
  { id: "scorings", title: "Rentang Penilaian", desc: "Klik 'Edit' pada kartu mana saja untuk mengubah rentang skor, nama label, dan pesan yang ditampilkan kepada pengguna setelah menyelesaikan asesmen." },
];

export default function AdminScriPenilaianPage() {
  ...
  useEffect(() => {
    ...
    async function fetchScoring() {
      setLoading(true);
      try {
        const snap = await getDocs(query(collection(db, "scri_scoring"), orderBy("order")));
        if (!snap.empty && !cancelled) {
          setScorings(snap.docs.map((d) => d.data() as ScriScoring));
        }
      } catch {
  ...
  function startEdit(s: ScriScoring) {
    setEditing(s.id);
    setEditForm({
      label: s.label,
      message: s.message,
  ...
  async function saveEdit(id: string) {
    setSaving(true);
    try {
      await updateDoc(doc(db, "scri_scoring", id), {
  ...
  async function handleSeed() {
    if (!confirm("Ini akan menambahkan atau menimpa data penilaian awal (6 rentang skor). Lanjutkan?")) return;
    setSeeding(true);
    try {
      const batch = writeBatch(db);
      for (const s of INITIAL_SCORING) {
        const id = `scoring_${s.order}`;
        batch.set(doc(db, "scri_scoring", id), { ...s, id });
      }
      await batch.commit();
      setRefreshKey((k) => k + 1);
      alert("Data penilaian awal berhasil dimuat!");
    } catch {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--foreground)]">Penilaian SCRI-36</h1>
      ...
      <AdminCopilot
        pageTitle="Penilaian SCRI-36"
        steps={SCRI_PENILAIAN_STEPS}
        youtubeUrl={ADMIN_TUTORIALS["scri-penilaian"]}
        storageKey="scri-penilaian"
      />
      ...
      <div data-copilot="info" className="bg-[var(--muted)]/50 border border-[var(--border)] rounded-xl p-4">
        ...
        <p className="text-sm text-[var(--foreground)]">
          Total skor maksimal: <strong>180</strong> &nbsp;·&nbsp; Jumlah pertanyaan: <strong>36</strong> &nbsp;·&nbsp;
          Skala jawaban: <strong>1–5</strong>
        </p>
      </div>
                ...
                {editing === s.id ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={saving}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--primary)] hover:opacity-90 text-white rounded-lg text-xs font-medium transition-opacity disabled:opacity-50"
                    >
                  ...
                  <>
                    ...
                    <div className="flex gap-4">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-24 px-3 py-1.5 text-sm bg-[var(--background)] border border-[var(--border)] rounded-lg text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                        />
                      </div>
                      <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        rows={4}
                        className="w-full px-3 py-2 text-sm bg-[var(--background)] border border-[var(--border)] rounded-lg text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] resize-none"
                      />

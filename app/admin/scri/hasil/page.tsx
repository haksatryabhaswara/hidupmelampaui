...
const SCRI_HASIL_STEPS: CopilotStep[] = [
  { id: "header", title: "Hasil SCRI-36", desc: "Semua hasil pengisian ditampilkan di sini. Klik tombol 'Export CSV' untuk mengunduh seluruh data ke spreadsheet." },
  { id: "stats", title: "Statistik Ringkas", desc: "Total pengisi, rata-rata skor, dan skor maksimal (180). Data diperbarui setiap halaman dimuat." },
...
function exportToCsv(results: ScriResult[]) {
  ...
  const rows = results.map((r) => [
    ...
    r.totalScore,
    r.scoringLabel,
    ...DIMENSIONS.map((d) => r.dimensionScores?.[d.id] ?? 0),
  ...
  a.href = url;
  a.download = `hasil-scri36-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
export default function AdminScri36HasilPage() {
  const [results, setResults] = useState<ScriResult[]>([]);
    ...
    async function load() {
      ...
      try {
        const snap = await getDocs(
          query(collection(db, "scri_results"), orderBy("completedAt", "desc")),
        );
        if (!cancelled) {
          setResults(
            snap.docs.map((d) => ({ ...(d.data() as Omit<ScriResult, "id">), id: d.id })),
          );
        }
      } catch {
    ...
    <div className="p-6 space-y-6">
      ...
      <div data-copilot="header" className="flex items-start justify-between flex-wrap gap-3">
        <div className="space-y-1">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Kembali ke SCRI-36
          </Link>
          <h1 className="text-xl font-bold text-[var(--foreground)]">Hasil SCRI-36</h1>
          <p className="text-sm text-[var(--muted-foreground)]">
            Semua pengisian Self-Command Readiness Index — 36 pertanyaan
          </p>
        </div>
        {results.length > 0 && (
          <button
      ...
      <AdminCopilot
        pageTitle="Hasil SCRI-36"
        steps={SCRI_HASIL_STEPS}
        youtubeUrl={ADMIN_TUTORIALS["scri-hasil"]}
        storageKey="scri-hasil"
      />
      ...
      <div data-copilot="stats" className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-[var(--foreground)]">180</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-12 text-center text-[var(--muted-foreground)]">
          ...
          <p className="text-sm mt-1">Hasil akan muncul setelah pengguna menyelesaikan SCRI-36.</p>
        </div>
      ) : (

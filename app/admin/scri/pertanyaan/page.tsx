...
import { DIMENSIONS, type ScriQuestion } from "@/lib/scri-data";
...
function QuestionList() {
  ...
  useEffect(() => {
    ...
    async function fetchQuestions() {
      setLoading(true);
      try {
        const snap = await getDocs(collection(db, "scri_questions"));
        if (!cancelled) setQuestions(snap.docs.map((d) => d.data() as ScriQuestion));
      } catch (err) {
  ...
  async function handleDelete(id: string) {
    ...
    setDeleting(id);
    try {
      await deleteDoc(doc(db, "scri_questions", id));
  ...
  // Returns questions for a dimension sorted by order
  ...
  async function handleDrop(targetId: string, dimId: string) {
    ...
    try {
      ...
      withOrder.forEach((q) => batch.update(doc(db, "scri_questions", q.id), { order: q.order }));
      await batch.commit();
    } catch {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[var(--primary)] hover:opacity-90 text-white rounded-lg text-sm font-medium transition-opacity"
          >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm bg-[var(--card)] border border-[var(--border)] rounded-lg text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-sm bg-[var(--card)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-[var(--muted-foreground)]">
          ...
          <p className="text-sm">Belum ada pertanyaan. Tambahkan atau muat data awal dari halaman SCRI.</p>
        </div>
      ) : (
                ...
                {dimQs.map((q, idx) => (
                  <div
                    ...
                    className={`flex items-start gap-3 px-5 py-4 border-b border-[var(--border)] last:border-b-0 transition-colors ${
                      ...
                      dragOverId === q.id ? "bg-[var(--primary)]/5 border-l-2 border-l-[var(--primary)]" : "hover:bg-[var(--muted)]/40"
                    }`}
                  >
                    {isDimMode && (
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <Link
                        href={`/admin/scri/pertanyaan/${q.id}`}
...
// Export page with Suspense wrapper for useSearchParams
export default function AdminScriPertanyaanPage() {
  return (
    <Suspense
      fallback={
